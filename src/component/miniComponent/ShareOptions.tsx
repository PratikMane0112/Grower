...
>(({ acticleDetails, setShareOpen }, ref) => {
  const OPTIONS = [
    ...
    },
    {
      ...
      action: () => {
        window.open(
          `https://twitter.com/intent/tweet?url=${window.location.href}&text=${acticleDetails.title}&via=${acticleDetails.by} on @Grower`,
          "_blank"
        );
        setShareOpen(false);
      },
    ...
    },
    {
      ...
      action: () => {
        window.open(
          `https://www.linkedin.com/shareArticle?mini=true&url=${window.location.href}&title=${acticleDetails.title}&source=Grower`,
          "_blank"
        );
        setShareOpen(false);
      },
